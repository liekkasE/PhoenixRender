...
namespace FX {
	...
	void Buffer::updateBuffer()
	{
		...
		{
			GetD3DContext()->UpdateSubresource(m_GpuBuffer.m_gpu_addr, 0, nullptr, m_CpuAddr, m_CpuSize, 0);
		}
		...
		void* data /*= nullptr*/)
	{
		...
		Desc.ByteWidth = noofElements * elementSize;
		Desc.MiscFlags = 0;
		...
		if (bindFlags & BufferBindFlags::SRV)
		{
			...
			shaderResourceViewDesc.Buffer.NumElements = noofElements;
		...
		if (bindFlags & BufferBindFlags::UAV)
		{
			...
			unorderedAccessViewDesc.Buffer.NumElements = noofElements;
